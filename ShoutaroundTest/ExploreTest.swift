...
class ExploreControllerOLD: UIViewController, UISearchBarDelegate, HomePostSearchDelegate, UISearchControllerDelegate, FilterControllerDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, GridPhotoCellDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(finishFetchingPostIds), name: ExploreControllerOLD.finishFetchingRankPostIdsNotificationName, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(finishPaginationCheck), name: ExploreControllerOLD.finishPaginationRankPostIdsNotificationName, object: nil)
    ...
    fileprivate func fetchingPostIds(){
        ...
        query.observe(.value, with: { (snapshot) in
            ...
            postIds.forEach({ (key,value) in
                ...
                var tempPostId = PostId.init(id: key, creatorUID: nil, sort: varSort)
            ...
            NotificationCenter.default.post(name: ExploreControllerOLD.finishFetchingRankPostIdsNotificationName, object: nil)
                ...
                if let error = error {
                    ...
                    self.filterPosts(post: fetchedPost!, completion: { (filteredPost, filterCondition) in
                        ...
                        if self.fetchedPostCount == paginateFetchPostsLimit {
                            ...
                            NotificationCenter.default.post(name: ExploreControllerOLD.finishPaginationRankPostIdsNotificationName, object: nil)
                        }
                    })
                }
            })
